...
function isCreditCardNumber(ccn) {
  const digits = Array.from(String(ccn), Number);
  ...
  const lessThan18 = digits.map((num) => (num > 9 ? num - 9 : num));
  const sum = lessThan18.reduce((acc, elm) => acc + elm, 0);
  return sum % 10 === 0;
}
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i <= pairs.length; i += 1) {
    while (newStr.indexOf(pairs[i]) !== -1) {
      newStr = newStr.replace(pairs[i], '');
      i = 0;
    }
  }
